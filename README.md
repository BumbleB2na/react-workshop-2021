Working through exercises from a 2-day instructor led [corporate workshop](https://reacttraining.com/workshops) by React team. 

Changes going in origin/my-final branch, set as the default branch for this forked repository. Leaving origin/master alone to be able to pull latest from upstream/master. Then I can rebase origin/my-final with origin/master. Usually do work in a separate branch then make a new Pull Request, being careful to set base:my-final as the target branch. I like to use the big flashy github buttons to create a new PR from a branch but it likes to default base target to upstream master.
